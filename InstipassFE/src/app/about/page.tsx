  ...
  return (
    <div className={`flex flex-col min-h-screen ${darkMode ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'} overflow-x-hidden`}>
      ...
      <main className="flex-grow overflow-x-hidden">
        ...
        <section className={`py-20 ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`} id='testimonials'>
          <div className="container mx-auto px-6">
            <div className="text-center mb-16">
              <motion.h2 
                className="text-3xl md:text-4xl font-bold mb-4"
                ...
                viewport={{ once: true }}
              >
                What Our <span className="text-[#2A9D8F]">Clients Say</span>
              </motion.h2>
              <motion.div 
                className="w-24 h-1 bg-[#2A9D8F] mx-auto mb-6"
                initial={{ width: 0 }}
            ...
            <div className="grid md:grid-cols-3 gap-8">
              {testimonials.map((testimonial, index) => (
                <motion.div
                  ...
                  viewport={{ once: true }}
                >
                  <div className="mb-4 h-16 w-16 bg-gray-200 rounded-full flex items-center justify-center">
                    <p className="text-xs text-gray-500">Logo</p>
                    {/* Replace with actual logo */}
                  </div>
                  <div className="mb-4">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Star key={star} size={16} className="inline-block text-yellow-400 mr-1" fill="#FBBF24" />
                    ))}
                  </div>
            ...
            <div className="text-center mt-12">
              <motion.button
                onClick={() => setIsBookDemoOpen(true)}
                className="px-6 py-3 bg-[#2A9D8F] text-white rounded-full hover:bg-opacity-90 transition-all inline-flex items-center justify-center"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
                <Calendar className="mr-2" size={20} />
